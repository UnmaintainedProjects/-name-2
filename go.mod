module github.com/gotgcalls/bot

go 1.17

require (
	github.com/PaulSonOfLars/gotgbot/v2 v2.0.0-rc.5
	github.com/gotd/td v0.54.1
	github.com/gotgcalls/converter v0.0.0-20220117122821-2b2ac91f8939
	github.com/gotgcalls/tgcalls v0.0.0-20220117140119-88cae7467479
	github.com/joho/godotenv v1.4.0
)

require (
	github.com/cenkalti/backoff/v4 v4.1.2 // indirect
	github.com/go-faster/errors v0.5.0 // indirect
	github.com/go-faster/jx v0.25.0 // indirect
	github.com/go-faster/xor v0.3.0 // indirect
	github.com/gotd/ige v0.2.2 // indirect
	github.com/gotd/neo v0.1.5 // indirect
	github.com/klauspost/compress v1.13.6 // indirect
	go.uber.org/atomic v1.9.0 // indirect
	go.uber.org/multierr v1.7.0 // indirect
	go.uber.org/zap v1.19.1 // indirect
	golang.org/x/crypto v0.0.0-20200622213623-75b288015ac9 // indirect
	golang.org/x/net v0.0.0-20211015210444-4f30a5c0130f // indirect
	golang.org/x/sync v0.0.0-20210220032951-036812b2e83c // indirect
	nhooyr.io/websocket v1.8.7 // indirect
	rsc.io/qr v0.2.0 // indirect
)
